...
contract SmartProject {
    ...
    function SmartProject() {
        owner = msg.sender;
        CallForTender memory callForTender = CallForTender({deadline:"deadline le 21", offers:"['offer1, 'offer2']"});
    ...
    function tenderToJson(CallForTender callForTender) private returns(string){
        return "{".toSlice().concat("deadline:".toSlice())
            ...
            .concat(",offers:".toSlice()).toSlice().concat(callForTender.offers.toSlice());
    }
