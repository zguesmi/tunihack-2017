...
contract SmartProject {
    ...
    function SmartProject() {
        ...
        string memory offer1 = "{sender:address(0), description:offer1}";
        string memory offer2 = "{sender:address(0), description:offer2}";
        offers[0] = offer1;
        offers[1] = offer2;
        CallForTender memory callForTender = CallForTender({deadline:"zied dead",offers:offers});
    ...
    function  projectToJson(Project project) private returns(string) {
        ...
        return stackTooDeepHelp(project, pr, str1);
    }
    ...
    function tenderToJson(CallForTender callForTender) private returns(string){
        return "{".toSlice().concat("deadline:".toSlice())
            ...
            .concat(",offers:".toSlice()).toSlice().concat(offersToJson(callForTender.offers).toSlice());
    }
    function offersToJson(string [] offers) private returns (string) {
        string memory str = "[" ;
        for(uint i = 0; i<offers.length; i++) {
            str = str.toSlice().concat(offers[i].toSlice());
            str = str.toSlice().concat(",".toSlice());
        }
        str = str.toSlice().concat("]".toSlice());
        return str;
